...
from schemas import CorpusCreate, QueryRequest, UserCreate, UserLogin
...
def save_chat_message(user_id: int, question: str, answer: str) -> dict[str, Any]:
    with get_connection() as conn:
        cursor = conn.execute(
            """
...
def simple_answer(corpus_id: int, question: str) -> str:
    return f"Received your question for corpus {corpus_id}: {question}"
...
async def upload_document(
    ...
    del user_id
    return save_document(corpus_id, file.filename or "document.txt", await file.read())
...
def query_corpus(
    ...
    answer = simple_answer(corpus_id, query.question)
    save_chat_message(user_id, query.question, answer)
...
from routes.query_routes import (
